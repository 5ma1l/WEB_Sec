...
no_chars=find_number_of_chars(s,url,cookies,"Internal Server Error",type_condition=False)
...
for i in range(no_chars):
    ...
    for j in alpha_num:
        res=it_this_char(s,url,i+1,j,cookies,"Internal Server Error",type_condition=False)
        if res[0]:
            print(res[1])
            pass_code+=res[1]
            break
